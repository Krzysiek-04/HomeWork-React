...
const CharactersList = ({ setMainCharacters }) => {
  ...
  return (
    <Container>
      <Head>
        {" "}
        LISTA POSTACI {page}/{characters.info.pages}
      </Head>
      <PagesButtons>
        <Button onClick={prevPage} style={{ margin: 5 }} variant="contained">
          Poprzednia
        </Button>
        <Button onClick={firstPage} style={{ margin: 5 }} variant="contained">
          Pierwsza
        </Button>
        <Button onClick={nextPage} style={{ margin: 5 }} variant="contained">
          Następna
        </Button>
      </PagesButtons>
      ...
      <FiltersBlock>
        ...
        <FormControlLabel
          control={
            <Switch onChange={(e) => setAlphabeticalOrder(e.target.checked)} />
          }
          label="A-Z"
        />
      </FiltersBlock>
